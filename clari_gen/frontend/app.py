...
with col1:
    ...
    if st.button("Analyze Query", type="primary"):
        with st.spinner("Analyzing..."):
            try:
                ...
                st.session_state.chat_history = [{"role": "user", "content": query_text}]
                ...
                st.session_state.api_calls = [st.session_state.api_calls[-1]] # Keep only the one we just added
    ...
    # Clarification Input (Only enabled if context exists)
            ...
            if not clarification_answer:
                ...
                with st.spinner("Reformulating..."):
                    try:
                        ...
                        st.session_state.chat_history.append({"role": "user", "content": clarification_answer})
