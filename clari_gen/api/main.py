...
from clari_gen.api.schemas import QueryRequest, ClarifyRequest, QueryResponse
...
async def process_clarification(request: ClarifyRequest):
    ...
    Returns the final reformulated query.
    """
    if not pipeline:
        raise HTTPException(status_code=500, detail="Pipeline not initialized")

    logger.info(f"Received clarification for query: {request.context.get('original_query', 'unknown')}")

    query_result = pipeline.continue_with_clarification(
        query_dict=request.context,
        user_clarification=request.answer
    )
