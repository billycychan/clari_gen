...
# Stop meta-llama/Llama-3.1-8B
if [ -f "$LOG_DIR/llama-3.1-8b.pid" ]; then
  PID=$(cat "$LOG_DIR/llama-3.1-8b.pid")
  if ps -p $PID > /dev/null 2>&1; then
    echo "Stopping meta-llama/Llama-3.1-8B (PID: $PID)..."
    kill $PID
    rm "$LOG_DIR/llama-3.1-8b.pid"
    echo "Stopped."
  else
    echo "meta-llama/Llama-3.1-8B is not running."
    rm "$LOG_DIR/llama-3.1-8b.pid"
  fi
else
  echo "No PID file found for meta-llama/Llama-3.1-8B"
fi
